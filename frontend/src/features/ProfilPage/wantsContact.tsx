...
import { CreateRatingRequest, RatingOpportunity, RatingRequest } from '../../types';
...
function WantsContact() {
  ...
  const acceptContact = async (id: number, postId: number, postTitle: string, firstId: number, secondId: number) => {
      ...
      <Container className='scroll' >
        <ToastContainer style={{width: "100%"}}>
          {ratingOpportunities.map((ro,idx) => (
            <Toast key={idx} show={show} onClose={toggleShow} style={{width: "500px"}} >
              <Toast.Header>
              ...
              <Toast.Body>
                {ro.contacterFirstName + " " + ro.contacterLastName} ønsker å {ro.forSale ? "kjøpe" : "selge"} <i>{ro.title}</i><br></br>
               <br></br>
                Ta kontakt for utføre salget: <a href={`mailto:  ${ro.contacterEmail}`}>{ro.contacterEmail}</a>
        
              </Toast.Body>
              
              <Button onClick={() => {
                acceptContact(ro.id, ro.postId, ro.title, ro.contactedId, ro.contacterId)}}>
